...
case class TrainingJob2(jn: String, model: com.ai.relpredict.spark.Model, conf : Config, ss : SparkSession, df : DataFrame, dm: Datamap, jobParms : Map[String, String])
   extends Job(jn: String, model: Model, conf : Config, jobParms : Map[String, String]) {
    import ss.sqlContext.implicits._
    def run() : Map[String, Any] = {
      r_job.addString("job_name", jn);
      r_job.addString("run_date", ScalaUtil.getDateTimeString())
      r_job.addString("config", conf.getConfString())
      resArray("job") = r_job.convertToMap();
      val split = {
      ...
      model.targets.foreach(t => {
        ...
        t.algorithms.foreach(a => {
          ...
          r_data.addString(s"job.train_model.${model.name}.${t.getName()}.${a.get.name}.status.msg", trainMsg)
          r = r.merge(rTrain)
